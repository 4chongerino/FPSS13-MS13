...
/proc/random_zone()
	return pick("chest","head","arms","legs")


/proc/zone_scatter(zone = null, probability = 90)
...
/proc/stars(text, readability=25, star="*")
	if(readability >= 100)
		return text
	...
	for(var/i=1, i<=len, ++i)
		var/ascii = text2ascii(text, i)
		if(ascii == 32 || prob(readability))
			. += ascii2text(ascii)
		else
			. += star
	return
...
/mob/proc/abiotic(var/full_body = 0)
	if(full_body && (src.l_hand || src.r_hand || src.back || src.wear_mask))
		return 1
